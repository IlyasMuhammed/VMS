...
/// <summary>Allocates <c>INV-YYYY-NNNNN</c> numbers (§32.4). Temporary, per CC-26's own note in the task register
/// ("Until answered, CC-25 may use a temporary INV-YYYY-NNNNN sequence behind the generator interface") — Q1
/// (numbering format, per-customer prefix, fiscal-year reset) is still open, so this interface is exactly the
/// seam CC-26 will implement against once it's answered, without touching <see cref="InvoiceCreationService"/>.</summary>
...
internal sealed class InvoiceNumberAllocator(TripsDbContext db, ITenantContext tenant, IOperatingClock clock) : IInvoiceNumberAllocator
{
    public async Task<string> NextAsync(CancellationToken ct = default)
    {
        ...
        return $"INV-{year}-{number.ToString(System.Globalization.CultureInfo.InvariantCulture).PadLeft(5, '0')}";
    }
}
